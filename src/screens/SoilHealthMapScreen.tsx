...
const SoilHealthMapScreen = ({ navigation }) => {
  ...
  // Alternative: Modal-based Picker Component (more reliable)
  const ModalPicker = ({
    label,
    selectedValue,
    onValueChange,
    ...
    return (
      <View style={styles.inputContainer}>
        ...
        <TouchableOpacity
          style={[styles.pickerTouchable, error && styles.errorInput]}
          onPress={() => {
            console.log('Opening picker for:', label);
            setIsVisible(true);
          }}
          testID={testID}
          activeOpacity={0.7}
            ...
            <View style={styles.pickerModal}>
              ...
              <ScrollView style={styles.pickerOptions}>
                {options.map((option, index) => (
                  <TouchableOpacity
                    key={`${testID}-option-${index}`}
                    style={[
                      styles.pickerOption,
                      selectedValue === option && styles.selectedPickerOption
                    ]}
                    onPress={() => handleSelect(option)}
                  >
                    <Text style={[
                      styles.pickerOptionText,
                      selectedValue === option && styles.selectedPickerOptionText
                    ]}>
                      {option}
                    </Text>
  ...
  // Use this instead of CustomPicker if you want modal-based selection
  // Just replace CustomPicker with ModalPicker in the render section
  const CustomPicker = ({
    label,
    selectedValue,
    onValueChange,
    ...
    const showCrossPlatformPicker = () => {
      console.log('Picker touched for:', label);
    ...
    return (
      <View style={styles.inputContainer}>
        ...
        <TouchableOpacity
          style={[styles.pickerTouchable, error && styles.errorInput]}
          onPress={showCrossPlatformPicker}
          testID={testID}
          activeOpacity={0.7}
        >
          <Text style={[
  ...
  const reverseGeocode = async (latitude, longitude) => {
    try {
      const response = await fetch(
        ...
        {
          headers: {
            'User-Agent': 'SoilHealthMapApp/1.0'
          }
        }
      );
  ...
  const getCurrentLocation = async () => {
    ...
    Geolocation.getCurrentPosition(
      async (position) => {
        ...
        Alert.alert(
          'Location Found',
          `📍 ${address}\n\nYou can now draw geofencing boundaries on the map for soil health analysis.`,
          [{ text: 'OK' }]
        );
      },
      (error) => {
  ...
  const generateMapHTML = () => {
    ...
    <head>
        ...
        <title>Soil Health Geofencing Map</title>
    ...
    <body>
        <div class="info-panel">
            <div><strong>🗺️ Soil Health Mapping</strong></div>
        ...
        <script>
            ...
            function initMap() {
                ...
                map.on('click', function(e) {
                    if (coordinates.length < 4) {
                        ...
                        alert('Maximum 4 points allowed for soil health mapping!');
                    }
                });
            }
            ...
            function addPoint(lat, lng) {
                const marker = L.marker([lat, lng], {
                    icon: L.icon({
                        iconUrl: 'data:image/svg+xml;base64,' + btoa('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#8B5CF6"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>'),
                        iconSize: [30, 30],
                        iconAnchor: [15, 30]
                    })
                }).addTo(map);
            ...
            function applyGeofence() {
                if (coordinates.length < 3) {
                    alert('Please add at least 3 points to create a geofence for soil health mapping.');
                    return;
                }
  ...
  const handleMapMessage = (event) => {
    try {
      ...
      if (data.coordinates && data.area) {
        ...
        const locationString = `${data.location} (Mapped: ${data.coordinates.length} points)`;
        setFarmLocation(locationString);

        setMapModalVisible(false);

        Alert.alert(
          'Soil Health Mapping Complete! 🗺️',
          `✅ ${data.coordinates.length} boundary points mapped\n📐 Area: ${data.area} hectares\n📍 Location: ${data.location}\n\n⚡ Area has been automatically updated for soil analysis.`,
          [{ text: 'OK' }]
        );
      }
    } catch (error) {
  ...
  const fetchExpertReports = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/farmer/expert-reports/soil-health-map`, {
        method: 'GET',
        headers: {
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.reportsModal}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>Soil Health Reports</Text>
            <TouchableOpacity
          ...
          {loadingReports ? (
            <View style={styles.loadingContainer}>
              ...
              <Text style={styles.loadingText}>Loading soil health reports...</Text>
            </View>
              ...
              renderItem={({ item }) => (
                ...
                >
                  ...
                  <View style={styles.reportDetails}>
                    ...
                    <Text style={styles.reportType}>Soil Health Analysis</Text>
                  </View>
            ...
            <View style={styles.emptyReportsView}>
              <MaterialIcons name="terrain" size={48} color={colors.textSecondary} />
              <Text style={styles.emptyReportsText}>No soil health reports available</Text>
              <Text style={styles.emptyReportsSubtext}>
                Expert soil analysis reports will appear here once available
              </Text>
            </View>
          )}
        </View>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/farmer/service/soil-health-map`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-access-token': cleanedToken,
      ...
      if (response.ok) {
        ...
        navigation.navigate('SubmissionSuccess', {
          message: 'Your soil health mapping request has been successfully submitted!',
          navigateBackTo: 'Main',
        });
        // Reset form fields
        setFarmLocation('');
    ...
    <View style={styles.inputContainer}>
      ...
      >
        <TextInput
          ...
          placeholder="Tap to get current location & map soil area"
      ...
      {currentLocation && (
        ...
        >
          <MaterialIcons name="terrain" size={20} color={colors.white} />
          <Text style={styles.openMapButtonText}>Open Map for Soil Health Mapping</Text>
        </TouchableOpacity>
      )}
    </View>
  );
    ...
    >
      <View style={styles.mapModalContainer}>
        <View style={styles.mapHeader}>
          ...
          <Text style={styles.mapHeaderTitle}>Soil Health Mapping</Text>
          ...
          <WebView
            ...
            renderLoading={() => (
              <View style={styles.webViewLoading}>
                <ActivityIndicator size="large" color={blue} />
                <Text style={styles.loadingText}>Loading Soil Health Map...</Text>
              </View>
            )}
          />
        )}
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Soil Health Map</Text>
        <TouchableOpacity
          onPress={() => {
            fetchExpertReports();
      ...
      >
        ...
        <View style={styles.inputContainer}>
          ...
          <TextInput
            ...
            placeholder="Auto-filled from soil mapping or enter manually"
            placeholderTextColor="gray"
        ...
        <View style={styles.inputContainer}>
          ...
          <TextInput
            ...
            placeholder="Enter your soil health analysis query or specific requirements"
            placeholderTextColor="gray"
            multiline
            numberOfLines={4}
            returnKeyType="done"
        ...
        >
          {submitPending ? (
            ...
            <Text style={styles.submitButtonText}>Submit Soil Health Request</Text>
          )}
        </TouchableOpacity>
      </ScrollView>
      ...
      >
        {error ? error : 'Soil health mapping request submitted successfully!'}
      </Snackbar>
    </KeyboardAvoidingView>
  );
};
